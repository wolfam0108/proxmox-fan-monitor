        ...
        <div className="bg-slate-900/50 rounded-xl border border-slate-800 p-6">
            <div className="flex items-center justify-between mb-4">
                <div>
                    <h3 className="text-lg font-semibold text-cyan-400 flex items-center gap-2">
                        <span>🌡️</span>
                        {t('sensorManager.title')}
                    </h3>
                    <p className="text-slate-500 text-sm">
                        {sensors.length > 0
                            ? `${t('sensorManager.configured')} ${sensors.length}`
                            : t('sensorManager.notConfigured')}
                    </p>
                </div>
                <button
            ...
            {sensors.length === 0 ? (
                <div className="text-center py-8 text-slate-500 border border-dashed border-slate-700 rounded-lg">
                    ...
                    <p>{t('sensorManager.emptyTitle')}</p>
                    <p className="text-sm mt-1">{t('sensorManager.emptyDesc')}</p>
                </div>
            ) : (
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-slate-500">
                                            {sensor.type === 'hwmon' && `${sensor.paths?.length || 0} ${t('sensorManager.sensors')}`}
                                            {sensor.type === 'nvidia' && t('sensorManager.nvidia')}
                                            {sensor.type === 'drive' && `${sensor.devices?.length || 0} ${t('sensorManager.drives')}`}
                                        </div>
                                    </div>
                                </div>
